...
const { appConfig, updateConfig } = await import('../config.js');
...
test('config test uses request-scoped draft credentials without persisting them', async () => {
  const upstream = Fastify();
  ...
  upstream.post('/v1/responses', async (req, reply) => {
    ...
    capturedModel = (req.body as { model?: string }).model;

    if (capturedAuthorization !== 'Bearer draft-key') {
      reply.status(401);
  ...
  try {
    ...
    assert.equal(response.statusCode, 200);
    assert.equal(body.data.llm.connected, true, body.data.llm.error);
    assert.equal(capturedAuthorization, 'Bearer draft-key');
    assert.equal(capturedModel, 'draft-model');
    assert.equal(appConfig.llm.apiKey, 'saved-invalid-key');
    assert.equal(appConfig.llm.model, 'saved-model');
  } finally {
    await app.close();
    await upstream.close();
  }
