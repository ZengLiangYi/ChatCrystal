...
export async function configRoutes(app: FastifyInstance) {
  ...
  app.post('/api/config', async (req) => {
    ...
    if (body.embedding) {
      const newProvider = body.embedding.provider || appConfig.embedding.provider;
      const newModel = body.embedding.model || appConfig.embedding.model;
      const changed = newProvider !== appConfig.embedding.provider || newModel !== appConfig.embedding.model;
      ...
      if (changed) {
        ...
        if (embeddingCount > 0) {
          warnings.push(`切换 Embedding 模型将清空现有 ${embeddingCount} 条向量索引，需要重新生成`);
        }
      }
    }
    ...
    if (taskTracker.hasActiveTasks()) {
      ...
      warnings.push(`当前有 ${snapshot.active} 个任务进行中，切换后新任务将使用新 provider`);
    }
    ...
    if (body.embedding && body.confirm) {
      ...
      if (changed) {
        ...
        console.log('[Config] Cleared embeddings and vectra index due to model change');
      }
    }
    ...
    console.log(`[Config] Updated — LLM: ${appConfig.llm.provider}/${appConfig.llm.model}, Embedding: ${appConfig.embedding.provider}/${appConfig.embedding.model}`);
